...
const findBigPrices = (prices = [121, 41, 56, 1245]) => {
  const higherThanHundred = [];
  prices.forEach((price) => {
    if (price > 100) {
      higherThanHundred.push(price);
    }
  });
  return higherThanHundred;
};
 ...
 * @param {number[]} [] prices - árak tömbje.
 ...
 * @param {[Object]} [] employees - dolgozók.
 ...
 * EXPORTÁLD A FÜGGVÉNYT!
 */
const findHigherSalariedEmployees = (
  employees = [{ name: "", salary: 200000 }]
) => {
  const richEmp = [];
  employees.forEach((employee) => {
    if (employee.salary > 300000) {
      richEmp.push(employee);
    }
  });
  return richEmp;
};
 ...
 * EXPORTÁLD A FÜGGVÉNYT!
 */
const findEmployeesFromMarketing = (
  employees = [{ name: "", salary: 200000, dep: "marketing" }]
) => {
  const markEmp = [];
  employees.forEach((employee) => {
    if (employee.salary > 300000 && employee.dep === "marketing") {
      markEmp.push(employee);
    }
  });
  return markEmp;
};
 ...
 * EXPORTÁLD A FÜGGVÉNYT!
 */
const findGermanMigrants = (
  citizens = [{ nation: "German", email: "franz@gmail.com", city: "Budapest" }]
) => {
  const germansFromBp = [];
  citizens.forEach((citizen) => {
    if (
      citizen.nation === "German" &&
      citizen.email.includes("gmail") &&
      citizen.city === "Budapest"
    ) {
      germansFromBp.push(citizen);
    }
  });
  return germansFromBp;
};

export { findGermanMigrants };

export { copyFoodArray, findNamesWithA, findBigPrices };
