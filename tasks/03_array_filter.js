 ...
 */
const filterOldCars = (
  cars = [{ man: "Ford", year: 2000, type: "fiesta" }]
) => {
 ...
 * EXPORTÁLD A FÜGGVÉNYT!
 */
const findAdultCitizens = (
  citizens = [{ id: "33DDffgz", bd: 1987, nation: "Hungarian" }]
) => {
  const now = new Date().getFullYear();
  return citizens.filter(
    (cit) => now - cit.bd > 18 && cit.nation === "Hungarian"
  );
};
...
export { filterDietFoods };

export { fourCharFoods, findNamesWithA, findBigPrices };
