 ...
 */
const addNamePrefixToStudents = (
  students = [{ lastName: "Doe", gender: "male" }]
) => {
...
const getStudentReports = (
  students = [{ firstName: "John", lastName: "Doe", points: 20 }]
) => {
  return students.map((student) => {
    return `${student.firstName} ${student.lastName}, points: ${student.points}.`;
  });
};

export { getStudentReports };
...
const addStudentReports = (students = [{ firstName: "John", points: 20 }]) => {
  return students.map((student) => ({
    ...student,
    result: student.points > 50 ? "passing" : "not passing",
  }));
};

export { addStudentReports };
 ...
 * @desc EXPORTÁLD A FÜGGVÉNYT!
 */
const addCorrectedIncome = (
  employees = [{ name: "Bob", salary: 200000, premium: 1000, corr: 0.2 }]
) => {
  return employees.map((employee) => ({
    ...employee,
    income: (employee.salary + employee.premium) * (1 + employee.corr),
  }));
};

export { addCorrectedIncome };
/**
 * 7. feladat
 * @todo Pótold a hiányzó részeket!
...
const addDishPrices = (dishes = [{ id: 1, kcal: 101, type: "dessert" }]) => {
  return dishes.map((dish) => ({
    ...
    price:
      dish.type === "dessert"
        ? Math.round(200 - dish.kcal * 1.5)
        : Math.round(25 + dish.kcal),
  }));
};
